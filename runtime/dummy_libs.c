...
#ifdef DVPDVL_COUNT_RANDOM
#define RANDOM_DV_COUNT		dummy_libs_random(1, 10)
#define RANDOM_PDVL_COUNT	dummy_libs_random(2, 100)
#else
#define RANDOM_DV_COUNT	1
...
int d2v_get_document_vector( void *ctx, char *filename, int flag,  d2v_vector_t *vector)
{
    ...
    for( i = 0; i < len_vector; i++ ) {
        ...
        vector->elements[i].count = RANDOM_DV_COUNT;
    }
    return 0;
}
...
int pdvl_get( char *username, d2v_vector_t *pdvl, d2v_vector_t *filter )
{
    ...
    for( i = 0; i < len_pdvl_items; i++ ) {
        ...
        pdvl->elements[i].count = RANDOM_PDVL_COUNT;
    }
